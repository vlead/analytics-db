...
* Introduction
  This document will illustrate the design & data model of analytics database
  service for Virtual Labs platform.
...
** To store the feedback analytics of labs running on Openedx platform.
     ...
     platform. Every single feedback record is stored as single =json= record.
...
** To store the usage analytics of labs running on college cloud platform   
    ...
    - Example REST API ::
      ...
      #+END_EXAMPLE
** To store the user analytics of Virtual Labs Website
     :PROPERTIES:
     :CUSTOM_ID: user-analytics-vlabs
     :END: 
     Design to store the user analytics of the Openedx platform running Virtual
     Labs such as current logged in users, total registered users and total
     active users. User analytics are identified by the date

    - index ::
               vlabs
    - type ::
              openedx_user_analytics_<yyyy-mm-dd>

    - document ::
                  Sample =json= to get the current logged in users
       #+BEGIN_EXAMPLE
       {
          "nginx_log": "- - 183.82.186.105 - - [03/Sep/2017:15:38:49 +0530]  \"GET /dashboard HTTP/1.1\" 200 10958 0.217 \"https://vlabs.ac.in/login?next=/dashboard\" \"Mozilla/5.0 (X11; Linux x86_64; rv:45.0) Gecko/20100101 Firefox/45.0\"",
       ...
       #+BEGIN_EXAMPLE
       {
          "nginx_log": "- - 183.82.186.105 - - [03/Sep/2017:15:38:49 +0530]  \"GET /logout HTTP/1.1\" 200 10958 0.217 \"https://vlabs.ac.in/login?next=/dashboard\" \"Mozilla/5.0 (X11; Linux x86_64; rv:45.0) Gecko/20100101 Firefox/45.0\"",
          "clientip": "183.82.186.105",
          "time": "15:38:49",
          "method": "GET",
          "api_endpoint": "/logout",
    ...
    - Example REST API ::
      ...
      #+BEGIN_EXAMPLE
      http://localhost/vlabs/openedx_user_analytics_2017-08-20
      #+END_EXAMPLE
...
* Security
  Security for REST APIs of =elasticsearch= database service is designed as
  follows

    + GET ::
             Anyone from outside world can make GET requests to =elasticsearch=
...
<p align="center"> Analytics Database Security </p>
#+END_HTML
To edit the feedback relations image click [[https://docs.google.com/drawings/d/11GDV3JOzpr-CtfI_6r1BmP1xnuXBu0ty5x9Kzq68sJo/edit][here]]

  Above design illustrates the following
  - Only domain =vlabs.ac.in= or sub-domains =*.vlabs.ac.in= can make GET, PUT,
    POST and DELETE requests to =elasticsearch= database service APIs.
  - Other domains only can make =GET= requests to =elasticsearch= database
    service APIs
