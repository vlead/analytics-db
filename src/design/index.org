...
* Introduction
  This document will illustrate the design of analytics database service for
...
** To store the feedback analytics of labs running on Openedx platform.
     ...
     :END: 
     Design to store the feedback analytics of labs running Openedx platform
     (90 labs running as of Sep-01-2017)
    - index ::
               vlabs
...
** To store the user analytics of Virtual Labs Website
     ...
     Labs such as current logged in users and total registered users. And user
     analytics are identified by the date
    - index ::
               vlabs
    - type ::
...
** To store the user analytics of outreach portal
     ...
     as current logged in users and total registered users. And user analytics
...
* Security
  Security for =elasticsearch= database service is provided at upstream server
  level (i.e nginx)
  1. By default =elasticsearch= service runs on port =9200=
  2. To make elasticsearch GET APIs available to the public we setup the
     upstream web server (nginx) which runs on port =80= then forward all
     incoming =80= requests to =9200=
  3. Only =vlabs-analytics-api-service= domin/IP is allowed to make request on
     =POST=, =PUT= and =DELETE= methods =GET= method is allowed from everyone
* Design
#+BEGIN_HTML
