...
* Modified lab ids mapping
 ...
 [
  ...
  #+END_SRC
* Read the lines and replace them
  #+NAME: find-replace
  #+BEGIN_SRC python
def replace_str_in_file(file_name):
    ...
    with open(file_name, 'r') as file :
        filedata = file.read()
    lab-id-map-file = "map-lab-ids.json"
    with open(lab-id-map-file) as json_data:
        labs = json.load(json_data)
    for lab in labs:
        STR_TO_BE_SEARCHED = lab['old_lab_id']
        STR_TO_BE_REPLACED = lab['new_lab_id']
        filedata = filedata.replace(STR_TO_BE_SEARCHED, \
                                    STR_TO_BE_REPLACED)
    return filedata
                                
  #+END_SRC
 - Reference :: Convert csv to json [[http://www.csvjson.com/csv2json][referece]]
* Write into file
  #+NAME: write-to-file
  #+BEGIN_SRC python
...
if __name__== "__main__":
    ...
    file_data = replace_str_in_file(backup_file)
    write_to_file(backup_file, file_data):
#+END_SRC
...
<<main>>
#+END_SRC
#+BEGIN_SRC json :tangle map-lab-ids.json :eval no :noweb yes
<<labid-map-json>>
#+END_SRC
